...
export const Dashboard: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const filtered = invRes.data.data.filter((item: any) => 
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-400 font-medium">Loading system metrics...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-2xl bg-gradient-to-r from-slate-900 via-slate-900 to-blue-950 border border-slate-800">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Real-time status of your global product inventory, order queues, and return rates.</p>
        </div>
        <button 
          onClick={fetchData}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="p-6 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-between shadow-xl">
          <div>
            ...
            <h3 className="text-2xl font-bold text-white mt-1">
              ${metrics?.totalInventoryValuation.toLocaleString("en-US", { minimumFractionDigits: 2 })}
            </h3>
        ...
        <div className="p-6 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-between shadow-xl">
          <div>
            ...
            <h3 className="text-2xl font-bold mt-1 text-amber-500">{metrics?.lowStockCount}</h3>
        ...
        <div className="p-6 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-between shadow-xl">
          <div>
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Active Order Queue</span>
            <h3 className="text-2xl font-bold text-white mt-1">{metrics?.pendingOrders}</h3>
        ...
        <div className="p-6 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-between shadow-xl">
          <div>
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Completed Return Rate</span>
            <h3 className="text-2xl font-bold text-white mt-1">{metrics?.returnsRatePercentage}%</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 p-6 rounded-xl bg-slate-900 border border-slate-800 shadow-xl flex flex-col justify-between">
          <div>
            <h3 className="text-lg font-semibold text-white">Daily Sales Volumes</h3>
            <p className="text-xs text-slate-400 mt-1">Reflects gross daily orders in USD ($)</p>
          </div>
        ...
        <div className="p-6 rounded-xl bg-slate-900 border border-slate-800 shadow-xl flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">SKUs with critical stock shortages requiring actions.</p>
          </div>
          ...
          <div className="mt-6 flex-1 space-y-3">
            {criticalStock.length === 0 ? (
              ...
              criticalStock.map((item) => (
                <div key={item.id} className="flex items-center justify-between p-3 rounded-lg bg-slate-950 border border-slate-850">
                  ...
                  <div className="text-right">
                    <p className="text-xs font-bold text-slate-300">{item.quantity} units</p>
