...
export const createReturn = async (req: AuthenticatedRequest, res: Response) => {
  ...
  for (const item of items) {
    ...
    if (!orderLine) {
      return res.status(400).json({ error: `Product SKU ${item.sku} was not part of the original order.` });
    }
  ...
  const returnId = "ret_" + Math.random().toString(36).substring(2, 11).toUpperCase();
...
export const inspectReturn = async (req: AuthenticatedRequest, res: Response) => {
